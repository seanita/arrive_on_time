...
class Judge(object):
    ...
    leaves of the decision trees are most ripe for splitting and
    how that split should occur.

    Because it is a somewhat messy calculation and depends on the 
    data history, it needs to be initialized with and remember the 
    data set that it is associated with. Putting the evaluate() 
    function in its own class is the cleanest way to do this.
    """
    def __init__(self, arrival_times_df):
        """
        Parameters
        ----------
        arrival_times_dataframe: DataFrame
    ...
    def find_total_absolute_deviation(self, cols=None):
        ...
        total_deviation = np.sum(abs(actual_arrivals))
    ...
    def find_departure_time(self, eval_set_df):
        ...
        # Find the departure time that corresponds to lateness of 0.
