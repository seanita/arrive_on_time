...
class DecisionTree(object):
    """
    A decision tree for data with binary features, that is,
    where each feature has a value of either 0 or 1.
    """
    def __init__(self, err_fn=None, n_min=20, debug=False):
        ...
        err_fn: function
            ...
            error = err_fn(datestrs)

            where datestrs is a list of strings of YYYY-MM_DD format and
            error is a float.
        ...
        # They are included for east of interpreting the tree.
    ...
    def estimate(self, features):
        ...
        recommended_departure: int
            ...
            in minute from when work starts.
            (Negative means minutes before.)
        """
        if len(features) != len(self.root.features):
            if self.debug:
                print("The feature you are asking to estimate is a")
    ...
    def render(self):
        ...
        def plot_branches(level, x0, y_delta=0.2):
            """"
            ...
            x0: float
                x position of parent node
            y_delta: float
                Amount of y tail to be hidden
            
            Returns
            -------
            x_lo, x_hi: float 
                x positions of the low and high branch nodes
            """     
            y0 = -level
            y3 = -level - 1 
            y1 = y0 - y_delta
        ...
        def recurse(node, level, x):
            """
            Cycle through the tree in depth-first manner.
